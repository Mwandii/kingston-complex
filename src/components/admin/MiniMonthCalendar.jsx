    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        <button
          ...
          className="w-7 h-7 flex items-center justify-center rounded-md hover:bg-[color:var(--color-neutral-100)] text-[color:var(--color-neutral-500)]"
        >
          ‹
        </button>
      ...
      <div className="grid grid-cols-7 gap-1 text-center">
        {WEEKDAY_LABELS.map((label, index) => (
          <div key={`${label}-${index}`} className="text-[10px] text-[color:var(--color-neutral-400)] font-medium py-1">
            {label}
          </div>
        ))}
        ...
        {cells.map((day, index) => {
          ...
          return (
            <button
              ...
              className={`relative h-8 flex items-center justify-center text-[13px] rounded-md transition-colors ${
                isSelected
            ...
            >
              {day}
              {isBooked && !isSelected && (
                <span className="absolute bottom-1 w-1 h-1 rounded-full bg-[color:var(--color-accent-500)]" />
              )}
            </button>
          );
        })}
